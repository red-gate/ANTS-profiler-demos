...
namespace LibraryManager
{
    public partial class AdminConsole : Form
    {
        ...
        private async void UpdateStatus()
        {
            ...
            await UpdateStatusWithData();
        }

        private async Task UpdateStatusWithData()
        {
            using (var db = new LibraryManagerDatabase())
            {
                Status status = await db.GetStatus();
        ...
        private async void SearchForMembers()
        {
            ...
            IEnumerable<Member> members = await GetMembersFromDatabase(firstName, lastName, postCode, city, filterByLoan);
        ...
        private async void SearchForMagazines()
        {
            ...
            IEnumerable<Magazine> magazines = await GetMagazinesFromDatabase(title, author);
        ...
        private async void SearchForBooks()
        {
            ...
            if (archiveSearch)
            {
                await PopulateListIncludingArchiveSearch(title, author);
            }
            else
            {
                await PopulateListWithoutArchiveSearch(title, author);
            }
        ...
        private async Task PopulateListIncludingArchiveSearch(string title, string author)
        {
            IEnumerable<Book> books = await GetArchivedBooksFromDatabase(title, author);
            UpdateBookSearchResults(books);
        }

        private async Task<IEnumerable<Book>> GetArchivedBooksFromDatabase(string title, string author)
        {
            ...
            using (var connection = new SqlConnection(ConfigurationManager.ConnectionStrings["LibraryManagerDb"].ConnectionString))
            {
                ...
                using (var sqlCommand = new SqlCommand(queryString, connection))
                {
                    ...
                    return await InstantiateBooksFromQueryResultsAsync(sqlCommand);
                }
            }
        }

        private static async Task<IEnumerable<Book>> InstantiateBooksFromQueryResultsAsync(SqlCommand cmd)
        {
            using (var reader = await cmd.ExecuteReaderAsync())
            {
                var matchingBooks = new List<Book>();

                while (await reader.ReadAsync() && matchingBooks.Count() <= 100)
                {
        ...
        private async Task PopulateListWithoutArchiveSearch(string title, string author)
        {
            IEnumerable<Book> books = await GetUnarchivedBooksFromDatabase(title, author);
            UpdateBookSearchResults(books);
        }

        private async Task<IEnumerable<Book>> GetUnarchivedBooksFromDatabase(string title, string author)
        {
            using (var db = new LibraryManagerDatabase())
            {
                List<Book> books = await db.FindBooks(title, author);
                return books;
            }
        }
        ...
        private async void UpdateMemberDetails(int memberId)
        {
            ...
            using (var db = new LibraryManagerDatabase())
            {
                Member member = await db.FindMemberById(memberId);
                ...
                pictureBox_MemberDetailsImage.Image = await LookupUserImage(memberId);
            }
        ...
        private async Task<Bitmap> LookupUserImage(int memberId)
        {
            using (var wc = new WebClient())
            {
                var imgData = await wc.DownloadDataTaskAsync(ConfigurationSettings.AppSettings["ApiBaseUrl"] + "/Members/Images/" + memberId);
        ...
        private async void UpdateMagazineDetails(int magazineId)
        {
            label_Loading.Visible = true;
            panel_BookDetails.Visible = false;
            panel_MemberDetails.Visible = false;
            ...
            using (var db = new LibraryManagerDatabase())
            {
                Magazine magazine = await db.FindMagazineById(magazineId);
