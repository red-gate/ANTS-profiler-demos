...
namespace TicketDesk.Web.Client.Areas.Admin.Controllers
{
    ...
    public partial class SecurityManagementController : ApplicationController
    {
        ...
        [HttpGet]
        public virtual ActionResult GetUserByEmailAddress(string emailAddress)
        {
            string username = "";
            string connectionString = ConfigurationManager.ConnectionStrings["TicketDeskSecurityConnectionString"].ConnectionString;
            ...
            if (!String.IsNullOrEmpty(emailAddress))
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SqlCommand("aspnet_Membership_GetUserByEmail", conn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@ApplicationName", "TicketDesk");
                        cmd.Parameters.AddWithValue("@Email", emailAddress);
                        SqlParameter returnValue = cmd.Parameters.Add("@UserName", SqlDbType.NVarChar);
            ...
            {
                return RedirectToAction(MVC.Admin.SecurityManagement.EditUser(username));
            }
            return RedirectToAction(MVC.Admin.SecurityManagement.UsersList(0));
        }
    }
}